...
public final class AnneeScolaireDAO extends DAO<AnneeScolaire> {
		...
		try {
			setResultat(this.conn.createStatement(
			    ResultSet.TYPE_SCROLL_INSENSITIVE,
			        ResultSet.CONCUR_READ_ONLY).executeUpdate("INSERT INTO annee_scolaire (`id`,`annee`) VALUES ('"+obj.getId()+"','" +obj.getAnnee()+"')"));
	...
	public boolean update(AnneeScolaire obj) {
		try{
			setResultat(this.conn.createStatement(
            ResultSet.TYPE_SCROLL_INSENSITIVE,
                ResultSet.CONCUR_READ_ONLY).executeUpdate("UPDATE annee_scolaire SET annee='"+obj.getAnnee()+"'WHERE id= '"+  obj.getId()+ "'"));              
        }
        catch (SQLException e) {
            e.printStackTrace();
            }
